...
Parameters newParameters(uint64_t logN, const vector<uint64_t> &qi,
                         const vector<uint64_t> &pi, uint8_t logScale) {
  if (qi.size() > 255 || pi.size() > 255) {
    throw invalid_argument("newParameters requires <=255 primes of each type.");
  }
  return Parameters(lattigo_newParameters(logN, qi.data(), qi.size(), pi.data(),
                                          pi.size(), logScale));
}
